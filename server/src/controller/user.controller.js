...
const userController = {
  ...
  createUser: async (req, res, next) => {
    try {
      ...
      const { username, password, provider, phonenumber, avatar } = req.body;
      ...
      if (!provider) {
        ...
        // login
        query = "select * from tb_user where user_id = ? and is_deleted = ?";

        // query user data
        const [logInData] = await pool.execute(query, [data.insertId, 0]);
      ...
      if (data.length === 0) {
        ...
        const [getData] = await pool.execute(query, [dataInserted.insertId]);
  ...
  logIn: async (req, res, next) => {
    try {
      ...
      // compare password
      if (!provider) {
        query = "select * from tb_user where user_name = ? and is_deleted = ?";
        // query user data
        ...
        bcrypt.compare(password, data[0].password, (err, result) => {
          ...
          if (!result)
            return next(
              new appError(statusCode.FORBIDDEN, "incorect password")
            );
        });
  ...
  updateUser: async (req, res, next) => {
    try {
      const q =
        "UPDATE tb_user SET `user_name`=?,`phone_number`=?,`avatar`=?,`address`=? WHERE `user_id` = ?";
